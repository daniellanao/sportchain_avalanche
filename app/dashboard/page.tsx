...
// Mock data (later can come from Supabase or API)
const mockMetrics = {
  totalInvested: 12500,
              ...
              <table className="w-full">
                ...
                <tbody>
                  {mockInvestments.map((investment, index) => (
                    <tr key={investment.id} style={{ borderBottom: index < mockInvestments.length - 1 ? '1px solid rgba(220, 196, 142, 0.2)' : 'none' }}>
                      <td className="py-4 px-6 font-medium" style={{ color: 'var(--foreground)' }}>{investment.project}</td>
                      ...
                      <td className="py-4 px-6 text-center">
                        <button onClick={() => handleClaim(investment.id)} disabled={claimingId === investment.id || investment.claimable === 0} className="btn-gold px-4 py-2 rounded-lg text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed" style={{ color: 'var(--color-primary)' }}>
                          {claimingId === investment.id ? <><FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> Reclamando...</> : 'Reclamar Pago'}
                        </button>
                      </td>
                    </tr>
                  ))}
